...
// .wrangler/tmp/bundle-qz8w07/checked-fetch.js
...
// .wrangler/tmp/bundle-qz8w07/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-qz8w07/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-qz8w07/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
app.get("/api/crypto-symbols", async (c) => {
  try {
    const supabase = getSupabase(c.env);
    const { data, error } = await supabase.from("crypto_symbols").select("id, symbol, created_at, order_index").order("order_index");
    if (error) {
...
app.post("/api/crypto-symbols", async (c) => {
  try {
    ...
    const supabase = getSupabase(c.env);
    const { data: maxData } = await supabase.from("crypto_symbols").select("order_index").order("order_index", { ascending: false }).limit(1);
    const nextOrderIndex = maxData && maxData.length > 0 ? maxData[0].order_index + 1 : 0;
    const { data, error } = await supabase.from("crypto_symbols").insert({
...
app.delete("/api/crypto-symbols/:id", async (c) => {
  try {
    ...
    const supabase = getSupabase(c.env);
    const { error } = await supabase.from("crypto_symbols").delete().eq("id", id);
    if (error) {
      return c.json({ error: error.message }, 400);
    }
...
app.put("/api/crypto-symbols/reorder", async (c) => {
  try {
    ...
    const supabase = getSupabase(c.env);
    const updatePromises = symbols.map(
      (symbol, index2) => supabase.from("crypto_symbols").update({ order_index: index2 }).eq("id", symbol.id)
    );
